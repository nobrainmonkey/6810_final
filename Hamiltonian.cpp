...
double Hamiltonian::hamiltonian_periodic_ising_element(int i, int j, Eigen::MatrixXd *microstate_matrix_ptr, hamiltonian_param_struct *hamiltonian_param){
	...
	double element_interaction_energy = 
		-J * (
				(*microstate_matrix_ptr)(i,j) * (*microstate_matrix_ptr)(up, j) +  
...
double Hamiltonian::hamiltonian_periodic_ising_microstate(Eigen::MatrixXd *microstate_matrix_ptr, hamiltonian_param_struct *hamiltonian_param){
	int const rows = microstate_matrix_ptr -> rows();
	int const cols = microstate_matrix_ptr -> cols();
	...
	for(int i = 0; i < rows; i++){
		for(int j = 0; j < cols; j++){
			microstate_energy += hamiltonian_periodic_ising_element(i, j, microstate_matrix_ptr,  hamiltonian_param);
		}
	}
